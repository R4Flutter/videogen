...
import { MODULE_BY_PURPOSE, type VisualPurpose } from "./VisualPurpose.ts";
...
export const moduleRuns = (beats: ScriptBeat[]) => {
  const runs: { module: string; beats: number[] }[] = [];
  for (const b of beats) {
    const last = runs[runs.length - 1];
    if (last && last.module === b.module) last.beats.push(b.n);
    else runs.push({ module: b.module, beats: [b.n] });
  }
  return runs;
};
...
export const moduleCount = (beats: ScriptBeat[]) => {
  ...
  for (const b of beats) counts[b.module] = (counts[b.module] ?? 0) + 1;
  return counts;
};
...
export const enforceVariety = (
  ...
  for (let i = 1; i < beats.length; i++) {
    runLen = beats[i].module === beats[i - 1].module ? runLen + 1 : 1;
    const shouldSwap =
      runLen >= 3 || (runLen >= 2 && LOUD.has(beats[i].module) && i > 0 && i < beats.length - 1);
    if (shouldSwap) {
      const old = beats[i].module;
