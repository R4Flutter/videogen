...
const OVERLAY: [RegExp, string][] = [
  [/\b(purpose|rest|reflect)\b.*/, "narrative"],
  [/\b(question|reveal)\b/i, "narrative"],
  [/\b(emotion)\b/i, "attention"],
  [/\b(chapter)\b/i, "structure"],
];
...
export const buildDirectorPlan = (
  ...
  const rhythms = beats.map((b, i) => rhythmFor(b, facts[i], beats));
