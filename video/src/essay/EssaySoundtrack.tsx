...
const duck = (frame: number) => {
  ...
  const inWord = script.beats.some((b) => {
    ...
    return words.some((w) => ms >= w.start - 60 && ms < w.end + 60);
  });
...
const buildCurve = (plan: DirectorPlan) => {
  ...
  for (const e of plan.audioEvents) {
    if (e.kind === "music_level") knots.push({ at: e.at, value: e.value });
  }
    ...
    <>
      {plan.beats.map((b) => (
        ...
        >
          <Audio
            src={staticFile(
              `audio/vo/${voice.beats.find((t) => t.n === b.n)?.file ?? `beat-${b.n}.wav`}`,
            )}
            volume={0.95}
          />
        </Sequence>
      ...
      <Sequence from={0}>
        <Audio
          ...
          volume={(f) => 0.35 * duck(f) * level(f / fps)}
          loop
        />
      </Sequence>
    </>
